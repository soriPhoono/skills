...
| Skill | Description |
|---|---|
| [`frontmatter-linter`](skills/frontmatter-linter/SKILL.md) | Validate frontmatter across all vault note types (daily notes, wiki pages, raw sources, kanbans). Detects broken `[[wikilinks]]`, orphan pages, stale content, and cross-system inconsistencies. |
| [`session-logger`](skills/session-logger/SKILL.md) | Append structured worklog entries to today's daily note during a work session — project, tasks, files changed, status. |
| [`daily-note-manager`](skills/daily-note-manager/SKILL.md) | Create today's daily note from template, review yesterday's for completeness, validate section ordering. |
| [`tag-sanitizer`](skills/tag-sanitizer/SKILL.md) | Audit all vault tags: find near-duplicates, singletons, non-kebab-case tags. Propose and execute merges. |
| [`vault-git-sync`](skills/vault-git-sync/SKILL.md) | Stage grouped changes by vault area (daily/, wiki/, raw/, config/) and commit with conventional messages. |
| [`wiki-index-regenerator`](skills/wiki-index-regenerator/SKILL.md) | Rebuild `LLM-Wiki/wiki/index.md` from the actual filesystem — refresh topic tables, page counts, and statistics. |
